    ...
    if (endpoint.startsWith('/local-fs/')) {
      ...
      return withMounts(() => {
        ...
        if (localFsEndpoint === '/statSync') {
          const fullPath = `${mountPath}${body.path}`;
          const statFn =
            body.options?.followSymlinks === false ? fsSync.lstatSync : fsSync.statSync;
      ...
      return withMounts(async () => {
        ...
        if (asyncEndpoint === '/writeFile') {
          ...
          const { error } = await safeCallAsync(() =>
            fsPromises.writeFile(fullPath, body.content, fsOptions)
          );
          if (error) return Response.json({ ok: false, error }, { status: 500 });
          return Response.json({ ok: true, bytesWritten: body.content.length });
        }
        ...
        if (asyncEndpoint === '/readFile') {
          const fullPath = `${mountPath}${body.path}`;
          const { result: content, error } = await safeCallAsync(() =>
        ...
        if (asyncEndpoint === '/stat') {
          const fullPath = `${mountPath}${body.path}`;
          const statFn =
            body.options?.followSymlinks === false ? fsPromises.lstat : fsPromises.stat;
        ...
        if (asyncEndpoint === '/mkdir') {
          const fullPath = `${mountPath}${body.path}`;
          const { result, error } = await safeCallAsync(() =>
            fsPromises.mkdir(fullPath, body.options)
          );
          if (error) return Response.json({ ok: false, error }, { status: 500 });
          return Response.json({ ok: true, result });
        }
        ...
        if (asyncEndpoint === '/readdir') {
          ...
          const { result: entries, error } = await safeCallAsync(async () => {
            const dirents = await fsPromises.readdir(fullPath, {
              withFileTypes: true,
              ...body.options,
            });
            return dirents;
