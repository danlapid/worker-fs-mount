...
import type { DirEntry, SyncWorkerFilesystem } from './types.js';
 ...
 */
function copyDirectorySync(syncFs: SyncWorkerFilesystem, srcPath: string, destPath: string): void {
...
export function readFileSync(
  ...
  if (typeof path === 'number' && isVirtualFd(path)) {
    return readDescriptor(path, options);
  }
...
export function writeFileSync(
  ...
  if (typeof path === 'number' && isVirtualFd(path)) {
    writeDescriptor(path, data, options);
    return;
  }
  const pathStr = getPath(path);
  if (pathStr) {
...
export function appendFileSync(
  ...
  realFs.appendFileSync(path, data, options);
}
...
export function rmSync(
  ...
  if (pathStr) {
    ...
    if (match) {
      ...
      syncFs.rmSync(match.relativePath, { recursive, force });
      return;
    }
  }
...
export function rmdirSync(
  ...
  if (pathStr) {
    ...
    if (match) {
      ...
      syncFs.rmSync(match.relativePath, { recursive: false, force: false });
      return;
    }
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (realFs.rmdirSync as any)(path, options);
}
...
export function unlinkSync(path: Parameters<typeof realFs.unlinkSync>[0]): void {
  ...
  if (pathStr) {
    ...
    if (match) {
      ...
      syncFs.rmSync(match.relativePath);
      return;
    }
  }
...
export function renameSync(
  ...
  if (oldPathStr && newPathStr) {
    ...
    if (oldMatch && newMatch) {
      ...
      if (syncFs.renameSync) {
        syncFs.renameSync(oldMatch.relativePath, newMatch.relativePath);
        return;
      }
...
export function copyFileSync(
  ...
  realFs.copyFileSync(src, dest, mode);
}
...
export function symlinkSync(
  ...
  if (pathStr && targetStr) {
    ...
    if (match) {
      ...
      syncFs.symlinkSync(match.relativePath, targetStr);
      return;
    }
  }
  realFs.symlinkSync(target, path, type);
}
