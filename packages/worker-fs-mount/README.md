...
`worker-fs-mount/emscripten` can give an Emscripten NODERAWFS module a persistent
mount context. Build the module with `NODERAWFS`, `MODULARIZE`, and `EXPORT_ES6`,
and link the companion library:

```sh
emcc ... -sNODERAWFS -sMODULARIZE -sEXPORT_ES6 -sENVIRONMENT=node \
  -sASSERTIONS=0 \
  '-sDEFAULT_LIBRARY_FUNCS_TO_INCLUDE=["$workerFilesystem"]' \
  --js-library node_modules/worker-fs-mount/emscripten-library.js
```
...
const runtime = await createEmscriptenModule(
  '/data',
  new LocalDOFilesystem(ctx.storage),
  options => createApplication({ ...options, instantiateWasm })
);
await runtime.run(() => runtime.instance.start());
```

`createApplication` is the generated module factory; `instantiateWasm` supplies the
compiled Wasm module from the Worker bundle. Invoke exports through `run` so that
open descriptors retain their mount context on later requests. Each module has its
own working directory and standard-output buffers. Timers started in that context
inherit it. Files and directories use the normal mounted sync backend.

The companion library handles Emscripten's runtime `createRequire`, which bypasses
Wrangler aliases, and uses public `fs.constants` instead of `process.binding`.
Stdout/stderr use Emscripten's console callbacks, so `print` and `printErr` work.
`fd_sync` stays synchronous; an application checkpoint should separately await
`ctx.storage.sync()` for an explicit durability barrier.

This integration uses Emscripten JS-library hooks and requires a toolchain with
NODERAWFS support. It has been exercised with the Emscripten fork at
`21166256c4c4d73d39b3685c8973d7cbe427ce8c` in a Rust Minecraft server. The optional
entry point does not change other mounts or module environments.
