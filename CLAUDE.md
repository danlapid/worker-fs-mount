...
worker-fs-mount/
├── src/
│   ├── index.ts        # Entry point - exports public API (mount, findMount, etc.)
...
│   ├── memory-filesystem.ts  # In-memory WorkerFilesystem implementation
│   ├── package.json          # Workspace package with dependency on root
...
The tests use a **pnpm workspace** setup where `tests/` is a workspace package that depends on the root package. This demonstrates real-world usage where the alias references an npm package.

1. **`pnpm-workspace.yaml`** defines `tests/` as a workspace package

2. **`tests/package.json`** declares dependency on root:
   ```json
   {
     "dependencies": {
       "@cloudflare/worker-fs-mount": "workspace:*"
     }
   }
   ```

3. **`tests/wrangler.toml`** configures the alias to use the package:
   ```toml
   [alias]
   "node:fs/promises" = "@cloudflare/worker-fs-mount/fs"
   ```

4. **`tests/index.ts`** is the test worker that:
...
5. **`tests/index.test.ts`** spawns `wrangler dev` and makes HTTP requests

6. **`tests/memory-filesystem.ts`** is an in-memory WorkerFilesystem implementation

## Common Development Tasks

### Adding a New fs Method

1. Add the wrapped function to `fs-promises.ts` following the pattern:
   ```typescript
   ...
   ```
2. Add corresponding method to `WorkerFilesystem` interface in `types.ts`
3. Add test endpoint in `tests/index.ts`
4. Add test case in `tests/index.test.ts`
...
{
  "exports": {
    ".": "./dist/index.js",        // mount(), findMount(), etc.
