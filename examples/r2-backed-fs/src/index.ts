...
<head>
  ...
  <title>R2 Filesystem Explorer</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .breadcrumb button:hover { background: #3d2814; }
    .toolbar {
      display: flex;
      gap: 8px;
      margin-bottom: 16px;
    ...
    .file-item.selected { background: #3d2814; }
    .file-icon {
      width: 24px;
      margin-right: 12px;
      text-align: center;
...
<body>
  <div class="container">
    <h1>R2 Filesystem Explorer</h1>
    <p class="subtitle">Files are stored persistently in Cloudflare R2 object storage</p>

    <div class="info-box">
      This demo uses <code>node:fs/promises</code> APIs backed by R2 storage.
      All operations go through the standard fs API, which is aliased to <code>worker-fs-mount</code>.
      Files persist across requests and can store up to 5GB per file.
    </div>
